...
<!-- Sidebar - Desktop -->
<aside
	class="right-0x glass-3 transition-allx duration-300x fixed bottom-50 left-0 z-40 hidden h-2/3 rounded-2xl border-r pt-36 md:block"
	style="width: {isSidebarCollapsed ? '4rem' : '18rem'}"
	on:mouseenter={() => (isSidebarCollapsed = false)}
	on:mouseleave={() => (isSidebarCollapsed = true)}
>
	<div class="h-fullx m-auto overflow-y-auto px-3 py-4">
