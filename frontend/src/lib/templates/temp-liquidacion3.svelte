<script>
	...
	console.log('--LIQUIDACIÓN IMPUESTOS-- ');
...
{#if imageLoaded}
	...
	<div id="body" class=" border-gray m-auto h-[11in] w-[8.5in] border-x px-[0.5in]">
		...
		<div id="container factura" class="relative h-3/5 w-full border-2 border-black">
			<div
				style="top: -1rem;"
			...
			<div class="items-top flex w-full">
				<div class="w-full p-4">
							...
							{#each liqStore.tarifas as cont, index (cont.id || index)}
								<tr class="flex flex-row">
									<td class="basis-2/5 border-t border-r border-black p-0">
										<p class="mx-1 text-left text-[8px]">
											{cont.tramite}
										</p>
									</td>

									<td class="basis-2/5 border-t border-r border-black p-0">
										<p class="mx-1 text-[8px]">
											{cont.concepto}
										</p>
									</td>

									<td class="basis-1/5 border-t border-black p-0">
										<p class="mx-1 text-right text-[8px]">
											{cont.valor.toLocaleString(currency)}
										</p>
									</td>
								</tr>
							{/each}
					...
					<div class="my-3 flex flex-row items-center border-2 border-black">
						...
						<div class="mx-2 my-1 w-36 basis-1/4 rounded-full border border-black">
							<p class="px-3 text-right text-[11px]">
								${liqStore.valTotal.toLocaleString(currency)}
							</p>
						</div>
					</div>
				</div>
